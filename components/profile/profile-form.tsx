...
const canSave = useMemo(() => {
  ...
  return !!baseValid && !!unameValid && !busy && !uploading && !fileError
}, [user, displayName, username, isAvailable, nameErr, busy, uploading, fileError])
...
const onFileChange: React.ChangeEventHandler<HTMLInputElement> = (e) => {
  ...
  if (!ALLOWED_IMAGE_TYPES.includes(file.type as any)) {
    const allowed = "JPG, PNG, or WebP"
    const msg = `Unsupported file type. Please choose ${allowed}.`
    setFileError(msg)
  ...
  if (file.size > MAX_AVATAR_BYTES) {
    const msg = `File is too large. Max size is ${MAX_AVATAR_MB} MB (you selected ${formatBytes(file.size)}).`
    setFileError(msg)
    setSelectedFile(null)
    if (previewUrl) {
      URL.revokeObjectURL(previewUrl)
...
const uploadAvatar = async () => {
  ...
  if (fileError) {
    toast({ title: "Fix upload error", description: fileError, variant: "destructive" })
    return
  }
  setUploading(true)
  try {
